				...
				while ( (line=reader.ReadLine()) != null )
				{
					...
					try
					{
						if ( split.Length >= 5 )
						{
                            ...
                            Spell s = new Spell(split[0].Trim()[0], Convert.ToInt32(split[1].Trim()), Convert.ToInt32(split[2].Trim()), split[4].Trim(), reags);//split[3].Trim(), split[4].Trim(), reags );

							m_SpellsByID[s.GetID()] = s;

							if ( !string.IsNullOrEmpty(s.WordsOfPower) )
