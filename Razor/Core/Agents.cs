...
namespace Assistant
{
	...
	public class FriendsAgent : Agent
	{
		...
		{
			if ( m_Enabled )
				return m_Chars.Contains( ser ) || ( Config.GetBool( "AutoFriend" ) && (PacketHandlers.Party.Contains( ser ) || PacketHandlers.Faction.Contains(ser)) );
			else
				return false;
		}
