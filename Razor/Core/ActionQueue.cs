...
namespace Assistant
{
	...
	public class DragDropManager
	{
		...
		public static bool Drop( Item i, Mobile to, Layer layer )
		{
			...
			else
			{
				...
				if ( add )
				{
                    ...
                    Queue<DropReq> q;
                    if(!m_DropReqs.TryGetValue(i.Serial, out q) || q==null)
		...
		private static DropReq DequeueDropFor( Serial s )
		{
			DropReq dr = null;
            Queue<DropReq> q;
            if (m_DropReqs.TryGetValue(s, out q) && q != null)
			{
				if ( q.Count > 0 )
					dr = q.Dequeue();
				if ( q.Count <= 0 )
		...
		public static void GracefulStop()
		{
			...
			{
                Queue<DropReq> q;
                m_DropReqs.TryGetValue(m_Pending, out q);
				m_DropReqs.Clear();
	...
	public class ActionQueue
	{
		...
		private readonly static Queue m_Queue = new Queue();
		private readonly static ProcTimer m_Timer = new ProcTimer();
