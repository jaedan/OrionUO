...
namespace Assistant
{
	...
	public class Packet
	{
        ...
        public byte[] CopyBytes(long offset, long count)
        {
            byte[] read = new byte[count];
            for (long i = offset; i < offset + count && i < Length; i++)
                read[i] = (byte)m_Stream.ReadByte();
            return read;
        }
        ...
        public PacketReader GetCompressedReader()
        {
            ...
            if (fullLen >= 4)
            {
                ...
                {
                    if (ZLib.uncompress(buff, ref destLen, CopyBytes(m_Stream.Position, fullLen - 4), fullLen - 4) != ZLibError.Z_OK)
                    {
                        destLen = 0;
                        buff = new byte[1];
                    }
