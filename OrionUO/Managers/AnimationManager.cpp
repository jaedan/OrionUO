...
void CAnimationManager::InitIndexReplaces(puint verdata)
{
    ...
    WISP_FILE::CTextFileParser bodyParser(
        g_App.UOFilesPath("Body.def").c_str(), " \t", "#;//", "{}");
    WISP_FILE::CTextFileParser bodyconvParser(
        g_App.UOFilesPath("Bodyconv.def").c_str(), " \t", "#;//", "");
    WISP_FILE::CTextFileParser corpseParser(
...
void CAnimationManager::FixSittingDirection(uchar &layerDirection, bool &mirror, int &x, int &y)
{
    ...
    if (mirror)
    {
        if (Direction == 3)
        {
            y += 25 + data.MirrorOffsetY;//was 23
            x += offsX - 4;
        }
        else
        {
