...
void COrionWindow::DrawColorTitleBar(HTHEME hTheme, HDC hOutDC, bool maximized, LPCSTR str, int len, RECT orig)
{
	...
	if (hTheme)
	{
		...
		if (maximized)
		{
			needRegFill = DrawThemeBackground(hTheme, hDC, WP_MAXCAPTION, WindowActive ? MXCS_ACTIVE : MXCS_INACTIVE, &rect, NULL) != S_OK;
		}
		else
		{
			int modTop = GetSystemMetrics(SM_CYFRAME);
			rect.top -= modTop;
			needRegFill = DrawThemeBackground(hTheme, hDC, WP_CAPTION, WindowActive ? CS_ACTIVE : CS_INACTIVE, &rect, NULL) != S_OK;
			rect.top += modTop;
		}
	...
	if (needRegFill)
		FillRect(hDC, &rect, GetSysColorBrush(WindowActive ? COLOR_ACTIVECAPTION : COLOR_INACTIVECAPTION));
...
void COrionWindow::RedrawTitleBar()
{
	...
	if (curNCRP != newNCRP)
	{
		...
		DwmSetWindowAttribute(g_OrionWindow.Handle, DWMWA_NCRENDERING_POLICY, &policy, sizeof(policy));
		curNCRP = newNCRP;
	}
	...
	RECT rect;
	HDC hDC = GetWindowDC(g_OrionWindow.Handle);//funzione che permette di scrivere sulla sezione non interna, la titlebar...
